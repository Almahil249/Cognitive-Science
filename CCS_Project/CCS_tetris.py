...
def max_min_height(board):
    ...
    for col in range(board.shape[0]):  # Iterate over columns
        ...
        for row in range(board.shape[1] - 1, -1, -1):  # Iterate from bottom to top [24 -> 0]
            if board[col][row] != '.':
                height = 25 - row  # Calculate height from bottom
...
def deepest_valley_height(board):
    ...
    for col in range(1, board.shape[0] - 1):  # Iterate over columns 1 -> 9 (except the edges)
        left_height = 0
        right_height = 0

        # Calculate height of left neighboring column
        for row in range(board.shape[1] - 1, -1, -1): 
            if board[col - 1][row] != '.':
                left_height = 25 - row

        # Calculate height of right neighboring column
        for row in range(board.shape[1] - 1, -1, -1): 
            if board[col + 1][row] != '.':
                right_height = 25 - row

        # Calculate the valley height between the columns
        valley_height = min(left_height, right_height)
        if valley_height < min_valley_height:
            min_valley_height = valley_height

    return min_valley_height
